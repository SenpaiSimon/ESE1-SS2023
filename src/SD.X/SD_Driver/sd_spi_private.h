...
#define FILEIO_SD_MILLISECOND_DELAY     (unsigned int)((SYS_CLK_FrequencyInstructionGet()/FILEIO_SD_DELAY_PRESCALER/(unsigned int)1000) - FILEIO_SD_DELAY_OVERHEAD)


// Description: Media Response Delay Timeout Values
#define FILEIO_SD_NCR_TIMEOUT     (unsigned int)20          //uint8_t times before command response is expected (must be at least 8)
