...
//    		OpenSPI(SPI_START_CFG_1, SPI_START_CFG_2);
            ...
            #else
            unsigned int spiconvalue = 0x0003;
            unsigned int timeout;
...
//    		OpenSPI(SPI_START_CFG_1, SPI_START_CFG_2);
	...
	if(CSDResponse[0] & 0xC0)	//Check CSD_STRUCTURE field for v2+ struct device
	{
		...
		c_size = (((uint32_t)CSDResponse[7] & 0x3F) << 16) | ((unsigned int)CSDResponse[8] << 8) | CSDResponse[9];
	...
	else //if(CSDResponse[0] & 0xC0)	//Check CSD_STRUCTURE field for v1 struct device
	{
		...
		c_size = ((uint32_t)CSDResponse[6] << 16) | ((unsigned int)CSDResponse[7] << 8) | CSDResponse[8];	//Get the uint8_ts in the correct positions
		...
		c_size_mult = ((unsigned int)((CSDResponse[9] & 0x03) << 1)) | ((unsigned int)((CSDResponse[10] & 0x80) >> 7));
		...
		finalLBA = ((uint32_t)(c_size + 1) * (unsigned int)((unsigned int)1 << (c_size_mult + 2)) * block_len) - 1;	//-1 on end is correction factor, since LBA = 0 is valid.		
	}	
