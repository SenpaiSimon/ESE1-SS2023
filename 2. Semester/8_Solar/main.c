...
int main(void) {
    ...
    internalSwitchControl(closed);
    _delay_cycles(1000);

    while(1) {    
        internalSwitchControl(open);
        ...
        cycle++;
        if(cycle == 10) {
            // 100pF -- deltaT = tSample (4cycles) + tConv (14tAdc) -- 4uS + 14uS = 0.000018
            // mult by 10^6 to get uA to "normal" value
            // --> VoltValue * C/deltaT --> 100pF / 18uS * 100000000 = 555.55555555555555
            // will display 100 for 1.0uA
            setNumber((uint32_t)(adcStorage.ext * 555.5555555));
            cycle = 0;
        }
    }
}
