...
void initADC() {
    ...
    ADC12CTL0 = ADC12SHT0_0 | ADC12SHT1_0 | ADC12ON; // sample for 4 cycles -- 4uS at 1Mhz Clock
    // ADC12CTL1 = ADC12SSEL_3 | ADC12SHP | ADC12CONSEQ_1; // smclk as source -- autoscan mode
    ADC12CTL1 = ADC12SSEL_3 | ADC12SHP; // smclk as source -- single channel mode
    ...
    ADC12IER0 |= ADC12IE0; // only external channel rises interrupt
...
void internalSwitchControl(switch_state_t state) {
    if(state == closed) {
        P1SELC |= BIT3; // on to toggle sel to 0
        P1SELC &= ~BIT3; // toggle back to sel 1
        P1DIR |= BIT3; // output
        P1OUT &= ~BIT3; // output low
        // PJOUT |= BIT6; // led on
    } else {
        P1SELC |= BIT3; // on to toggle sel to 0
        P1SELC &= ~BIT3; // toggle back to sel 1
